...
def learn_decision_tree(dataset, target_name, feature_names, depth_limit):
    ...
    def decision_tree_learning(examples, attrs, parent_examples=(), depth=0):
        ...
        if(len(examples) == 0):
            return 0 # Replace with Pluraity value of parent_examples
        tree = None
        return tree
    ...
    def entropy(examples):
        ...
        entropy = 0
        for e in examples:
            prob_e = compute_probability(e, examples)
            print(prob_e)
            if prob_e == 0:
                entropy += 0
            else:
                entropy += prob_e * math.log(prob_e)
        return -1 * entropy

    def compute_probability(c, collection):
        sum = 0
        print(c)
        for x in collection:
            print(x)
    ...
    def information_gain(parent, children):
        """
        Takes a `parent` set and a set of subsets `children` of the parent.
        ...
        for child in children:
            prob_c = compute_probability(child, children)
            p += prob_c * entropy(c)

        return parent_entropy - p
